window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			compatChecking: "正在检查插件兼容性…",
			compatCheckFailed: "插件兼容性检查失败（不影响安装本身）。",
			compatFound: "⚠ 发现不兼容插件，升级后它们将无法加载（可先停用或等兼容版本）：",
			compatAllOk: "兼容性检查通过：已安装插件与目标版本兼容。",
			installedMarker: "当前",
			upgrading: "升级中",
			bannerIdle: "",
			history: "升级历史",
		...
		const en = {
			...
			compatChecking: "Checking plugin compatibility…",
			compatCheckFailed: "Plugin compatibility check failed (does not affect the install itself).",
			compatFound: "⚠ Incompatible plugins found — they will fail to load after the upgrade (disable them or wait for compatible releases):",
			compatAllOk: "Compatibility check passed: installed plugins are compatible with the target version.",
		...
		function VersionTab({ t }) {
			...
			const [compatCheck, setCompatCheck] = react.useState(null);
			...
			// 二次确认目标版本时，自动运行插件兼容性检查（目标 dsh-settings 是否缺少插件依赖的导出）
			react.useEffect(() => {
				if (typeof confirmTarget !== "string") return;
				const version = confirmTarget.startsWith("history:") ? confirmTarget.slice("history:".length) : confirmTarget;
				let alive = true;
				setCompatCheck({ loading: true });
				call(`/dsh-versions/compat?target=${encodeURIComponent(version)}`).then(
					(d) => { if (alive) setCompatCheck({ data: d }); },
					(e) => { if (alive) setCompatCheck({ error: e }); },
				);
				return () => { alive = false; };
			}, [confirmTarget]);
			if (loadError !== null) {
			...
			return h("section", { className: "dv_section" },
				...
				compatCheck !== null && compatCheck.loading
					? h("p", { className: "dv_message" }, t("compatChecking"))
					: null,
				compatCheck !== null && compatCheck.error
					? h("p", { className: "dv_message", "data-error": "true" }, t("compatCheckFailed"))
					: null,
				compatCheck !== null && compatCheck.data && Array.isArray(compatCheck.data.incompatible) && compatCheck.data.incompatible.length > 0
					? h("div", { className: "dv_banner", "data-state": "failed" },
						h("span", { className: "dv_bannerText" },
							t("compatFound") + " " + compatCheck.data.incompatible.map((r) => `${r.name}（缺 ${r.missing.join("/")}）`).join("；")))
					: null,
				compatCheck !== null && compatCheck.data && Array.isArray(compatCheck.data.incompatible) && compatCheck.data.incompatible.length === 0
					? h("p", { className: "dv_message" }, t("compatAllOk"))
					: null,
				actionError !== null
