 ...
 *   GET  /dsh-versions/updates         已装插件并发查 npm latest
 ...
 *   POST /dsh-versions/upgrade         安装指定版本（升级/降级/重装，备份 + 失败自动回滚）
...
const isEnginePlatform = () => ENGINE_PLATFORMS.includes(process.platform)
...
const UA = 'dsh-versions/0.2 (local dsh web instance)'
...
function fetchLatest(name) {
	...
	return fetchJsonUrl(url).then(
		(doc) => typeof doc?.version === 'string' ? doc.version : null,
		(error) => {
...
async function checkUpdates(ctx) {
	const frameworkInstalled = resolveFramework(ctx.baseUrl)?.version ?? null
	...
	const results = await Promise.all(checkable.map(async (p) => {
		...
		try {
			...
			if (latest === null) row.note = 'not-on-npm'
			else {
				row.latest = latest
	...
	try {
		frameworkLatest = await fetchLatest(FRAMEWORK_PKG)
	} catch {}
	updatesCache = { at: Date.now(), frameworkInstalled, frameworkLatest, results }
	return updatesCache
}
...
async function handleUpgrade(ctx, req, res) {
	...
	sendJson(res, 200, { ok: true, started: true, from, target, backupDir })
}
...
async function handle(ctx, req, res) {
	...
	if (method === 'GET' && pathname === `${ROUTE_PREFIX}/updates`) {
	...
	if (method === 'GET' && pathname === `${ROUTE_PREFIX}/upgrade-status`) {
		...
		if (st === null) sendJson(res, 200, { ok: true, status: 'idle' })
...
// 供测试/调试使用的生成器（不随路由暴露）。
export const __builders = { buildUpgradeScript, buildRelaunchScript }
